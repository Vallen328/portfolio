...
export default function Navbar() {
  ...
  const navLinks = useMemo(() => [
    ...
    { name: dict.nav.leetcode, href: "#leetcode" },
    { name: dict.nav.projects, href: "#projects" },
    { name: dict.nav.roadmap, href: "#roadmap" },
    { name: dict.nav.contact, href: "#contact" },
    { name: (dict.nav as Record<string, string>).resume || "Resume", href: "/Vallen_Dsouza_Resume.pdf", isDownload: true },
  ], [dict.nav]);
            ...
            {navLinks.map((link) => (
              <li key={link.name}>
                {link.isDownload ? (
                  <a
                    href={link.href}
                    download="Vallen_Dsouza_Resume.pdf"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="relative text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground transition-colors hover:text-foreground group py-2 flex items-center gap-1.5"
                  >
                    {link.name}
